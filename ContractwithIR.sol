...
//Basic functions
    ...
    function calculateInterest(uint256 _loanValue, uint256 _IR, uint256 _startTime) internal view returns (uint256) {
        uint256 elapsedTime = block.timestamp - _startTime;
        uint256 dailyInterestRate = _IR * 1e16 / SECONDS_PER_YEAR / 10; // APR converted to daily interest rate
        uint256 compoundedInterest = _loanValue * (1e18 + dailyInterestRate) ** elapsedTime / 1e18;
...
//Calculates accrued interest
      _accruedInterest = calculateInterest(_pendingAmount, IR, _loanValue);
